...
class MinesGame:
    ...
    def calculate_multiplier(self):
        ...
        not_opened = total_cells - len(self.opened)
        if not_opened <= 0:
            return self.multiplier
        safe_remaining = safe_cells - len(self.opened)
        if safe_remaining < 0:
            safe_remaining = 0
        probability = safe_remaining / not_opened
        if probability <= 0:
            return self.multiplier
        new_multiplier = self.multiplier * (1 / probability)
